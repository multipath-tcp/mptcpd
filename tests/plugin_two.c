...
static void plugin_two_new_connection(mptcpd_cid_t connection_id,
                                      ...
                                      struct mptcpd_pm *pm)
{
        ...
        assert(connection_id == test_cid_2);
...
static void plugin_two_new_address(mptcpd_cid_t connection_id,
...
static void plugin_two_new_subflow(mptcpd_cid_t connection_id,
                                   ...
                                   struct mptcpd_pm *pm)
{
        ...
        assert(laddr_id == test_laddr_id_2);
        assert(raddr_id == test_raddr_id_2);
        assert(!mptcpd_addr_is_equal(laddr, raddr));
        assert(mptcpd_addr_is_equal(laddr, &test_laddr_2));
        assert(mptcpd_addr_is_equal(raddr, &test_raddr_2));
...
static void plugin_two_subflow_closed(mptcpd_cid_t connection_id,
                                      struct mptcpd_addr const *laddr,
                                      struct mptcpd_addr const *raddr,
                                      struct mptcpd_pm *pm)
{
...
static void plugin_two_connection_closed(mptcpd_cid_t connection_id,
                                         struct mptcpd_pm *pm)
{
        (void) pm;
...
{
        assert(plugin_call_count_is_sane(&call_count, &test_count_2));
